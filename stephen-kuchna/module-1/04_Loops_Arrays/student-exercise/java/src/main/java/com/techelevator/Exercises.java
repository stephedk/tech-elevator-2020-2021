...
public class Exercises {
	...
	public boolean firstLast6(int[] nums)
	{
		...
		boolean isSixFirstOrLast = isSixFirst || isSixLast;
		
		return isSixFirstOrLast;
	...
	{
		boolean theResultIs = false;
		boolean arrayLengthAtLeastOne = nums.length >= 1;
		
		if (arrayLengthAtLeastOne)
		{
			
			theResultIs = nums[0] == nums[nums.length-1];
	...
	public int[] rotateLeft3(int[] nums) {
		int [] createdArray = new int [3];
		createdArray [0] = nums[1];
		createdArray [1] = nums[2];
		createdArray [2] = nums [0];
		return createdArray;
	}
	...
	public int[] maxEnd3(int[] nums) 
	{
		...
		int [] newFigureLargest = new int [3];
			newFigureLargest [0]= biggest;
			newFigureLargest [1]= biggest;
			newFigureLargest [2]= biggest;
			{
			return newFigureLargest;
			}
		}
