...
public class Exercises {
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		Map<Integer, Integer> crunkJuice = new HashMap<>();
		
		if (ints != null) {
			
		for (Integer boop : ints) {
			if(!crunkJuice.containsKey(boop)) {
				crunkJuice.put(boop, 1);
			}
			else {
				crunkJuice.put(boop, crunkJuice.get(boop) +1);
			}
		}
	}
	...
	public Map<String, Boolean> wordMultiple(String[] words) {
		...
		Map<String, Boolean> aBunchOfWords = new HashMap<>();
		
		if (words != null) {
			
		for(String word : words) {
			if(!aBunchOfWords.containsKey(word)){
				aBunchOfWords.put(word, false);
			}

			else {
				aBunchOfWords.put(word, true);
			}
		}
			
	}
		return aBunchOfWords;
	}
